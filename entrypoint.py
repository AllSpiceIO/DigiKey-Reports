...
DIGIKEY_API_V4_KEYWORD_SEARCH_ENDPOINT      = DIGIKEY_API_URL_BASE + "/products/v4/search/keyword"
...
def query_digikey_v4_API_keyword_search(url, client_id, access_token,
    ...
    keyword_search_json = response.json() if response.status_code == 200 else response.text
...
def extract_data_from_digikey_search_response(keyword_search_json):
    # Initialize a part details object to store the extracted info
    part_data = PartDetails()
    # Extract product data from exact match
    product_data = (keyword_search_json["ExactMatches"])[0] if keyword_search_json["ExactMatches"] else None
    ...
    if product_data:
        ...
        pricing_variations = [variation['PackageType']['Name'] for variation in part_data.pricing]
        cut_tape_idx = digi_reel_idx = [i for i in range(0,len(pricing_variations)) if "Cut Tape" in pricing_variations[i]]
        digi_reel_idx = [i for i in range(0,len(pricing_variations)) if "Digi-Reel" in pricing_variations[i]]
        if(cut_tape_idx and digi_reel_idx):
            part_data.pricing[cut_tape_idx[0]]['PackageType']['Name'] = "Cut Tape (CT) & Digi-Reel®"
            del(part_data.pricing[digi_reel_idx[0]])
        # Initialize part parameter variables
        part_data.package_case = part_data.supplier_device_package = part_operating_temp = part_xy_size = part_height = part_thickness = None
...
if __name__ == "__main__":
    ...
    with open(args.bom_file, newline='') as bomfile:
        ...
        # Save as a list
        bom_line_items = (list(bomreader))[0:5]
        # Skip the header
    ...
    (response_code, access_token) = get_access_token(DIGIKEY_API_AUTH_ENDPOINT, digikey_client_id, digikey_client_secret)
    ...
    print(args.output_path + "/component_report.zip")
    shutil.make_archive(args.output_path + "/component_report.zip", format='zip', root_dir="/component_report")
